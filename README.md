# mask2dataset (DeepLabV3 Cityscapes)

360°の画像フォルダ or 360°動画から、DeepLabV3 (Cityscapes)でセグメンテーションした学習用データセットを生成します。
...
- **初回実行時は DeepLabV3 (Cityscapes) のモデルをダウンロード**します（ネット接続と十分なディスク容量が必要）。
